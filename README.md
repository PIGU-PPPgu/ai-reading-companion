...
<td><img src="screenshots/graph-with-tree.png" alt="Knowledge Graph with Tree" width="400"/></td>
</tr>
<tr>
<td><img src="screenshots/full-math-tooltip.png" alt="Knowledge Graph Node Tooltip" width="400"/></td>
<td><img src="screenshots/graph-with-expand.png" alt="Knowledge Graph Expanded" width="400"/></td>
</tr>
</table>
...
| 🕸️ **Knowledge Graph** | 交互式知识图谱 | D3.js，掌握度追踪，1500+节点，小地图·双击聚焦邻域·PNG导出·全屏·面包屑·键盘导航 |
...
| 📖 **Content Manager** | 内容导入管理 | PDF/EPUB/URL 真实提取 ✅ |
...
- **Knowledge Graph UX 第二轮增强（2026-04-20）**: 小地图、双击聚焦邻域、PNG导出、全屏、面包屑、节点hover tooltip、边hover高亮+标签、pulse动画、节点按连接数缩放、侧栏拖拽调宽、键盘导航、CSS柱状图、Top-5薄弱节点
...
# 4. 启动后端
python -m deeptutor serve --port 3001
...
| Phase 3 | 内容导入+前端优化 | 🚧 持续推进中：content_manager完成；知识图谱前端已完成第二轮 UX 增强 |
| Phase 4 | 打磨+测试 | ⏳ 进行中 |
| Phase 5 | 上线准备 | 📋 待开始 |

## 最近更新（2026-04）

- **知识图谱第二轮 UX 增强**：平滑缩放/平移过渡、节点悬停 Tooltip（标签+掌握度%）、选中节点 pulse 动画、边悬停高亮+关系标签、节点大小按连接数缩放、小地图（大图谱全局视图）、可折叠树面板、拖拽调整侧栏宽度、节点数 Badge、键盘导航（↑↓ 箭头+Enter）、CSS 掌握分布柱状图、Top-5 最薄弱节点、双击节点聚焦邻域、PNG 导出、全屏切换、面包屑路径导航
- **Co-Writer TypeScript 修复**：补全 `MAX_PANEL_RATIO`/`MIN_PANEL_RATIO` 常量（分屏比例限制）、将 `editorYCacheRef` 声明提前至使用前（消除"块作用域变量在声明前使用"错误）、修复 `InteractiveGraph.tsx` D3 类型标注错误；`tsc --noEmit` 现零报错通过
- 修复 `socratic_dialog.py` 首轮上下文遗漏与提示词串味问题
- 修复 `memory_enhanced_chat.py` 直接修改 `context.metadata` 的共享上下文污染问题
- 修复 `audio_companion.py` 全局 `_podcast_tasks` 的最小并发防护与旧任务清理
- 知识图谱第一轮：掌握度筛选、薄弱节点快速定位、图/树/详情联动、自动聚焦高亮节点、树搜索高亮与默认展开
